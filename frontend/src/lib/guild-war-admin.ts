...
export type EquipmentSlotState = {
  key: string;
  /** 저장 시 보내는 장비 마스터 ID. 직접 입력이면 null. */
  equipmentId: number | null;
  /** 드롭다운에 없는 장비명. equipmentId가 있으면 무시한다. */
  customName: string;
};
    ...
    recommendations: detail.recommendations.map((recommendation) => {
      ...
      recommendation.attackTeamMembers.forEach((member) => {
        ...
        if (slot) {
          ...
          slot.equipmentSlots = (member.equipments ?? []).map((item) => ({
            key: crypto.randomUUID(),
            equipmentId: item.id ?? matchCatalogId(catalogs.equipments, item.name),
            customName: item.id == null ? item.name?.trim() ?? "" : "",
          }));
          if (slot.equipmentSlots.length === 0) {
            const legacyId = matchCatalogId(catalogs.equipments, member.equipmentSetName);
            if (legacyId != null) {
              slot.equipmentSlots = [{ key: crypto.randomUUID(), equipmentId: legacyId, customName: "" }];
            } else if (member.equipmentSetName?.trim()) {
              slot.equipmentSlots = [
                { key: crypto.randomUUID(), equipmentId: null, customName: member.equipmentSetName.trim() },
              ];
            }
          }
          slot.ringSlots = (member.rings ?? []).map((item) => ({
            key: crypto.randomUUID(),
          ...
          if (slot.ringSlots.length === 0) {
            ...
            } else if (member.ringName?.trim() || member.ringEnchantment?.trim()) {
              slot.ringSlots = [
                {
                  key: crypto.randomUUID(),
                  ringId: null,
                  customName: member.ringName?.trim() ?? "",
                  enchantment: member.ringEnchantment ?? "",
                },
              ];
            }
...
function toAttackTeamMembers(slots: TeamMemberSlot[]): AttackTeamMemberUpsert[] {
  return slots
    ...
    .map((slot) => ({
      ...
      equipments: slot.equipmentSlots
        .filter((item) => item.equipmentId != null || item.customName.trim().length > 0)
        .map((item) => ({
          equipmentId: item.equipmentId,
          customName: item.equipmentId == null ? normalizeOptionalText(item.customName) : null,
        })),
      rings: slot.ringSlots
        .filter((item) => item.ringId != null || item.customName.trim().length > 0)
        .map((item) => ({
          ringId: item.ringId,
          customName: item.ringId == null ? normalizeOptionalText(item.customName) : null,
